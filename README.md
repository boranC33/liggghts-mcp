...
| `validate_liggghts_bin(run_parse_probe=False)` | Probe `LIGGGHTS_BIN`: exists / executable / version line / capability flags `mesh_surface` and `mesh_surface_stress`. When `run_parse_probe=True`, additionally execute a self-contained 2-triangle STL + minimal `mesh/surface/stress` deck (`run 0`) inside a tempdir and report `mesh_surface_stress_parse_probe`, `parse_probe_exit_code`, `parse_probe_work_dir`, `parse_probe_error_tail`. The grep-based `mesh_surface_stress` flag can be wrong (some builds list the command in `-help` but reject it at parse time) — the parse probe is the authoritative check. The plate workflow REQUIRES `mesh_surface_stress_parse_probe=true`; the apt LIGGGHTS package has `mesh/surface` but **not** `mesh/surface/stress` |
...
For a stricter check that catches builds where `-help` lists the command but the parser rejects it, pass `run_parse_probe=True`. The server creates a tempdir with a 2-triangle STL and a minimal deck that uses `fix mesh/surface/stress` and `wall/gran ... mesh` then runs `run 0`. The probe is self-contained — no PINN dir or project STL involved. The tempdir is left on disk under `/tmp/liggghts_parse_probe_*` for debugging; check `parse_probe_error_tail` first if `mesh_surface_stress_parse_probe` is `false`.
...
| `validate_liggghts_bin(run_parse_probe=False)` | 探测 `LIGGGHTS_BIN`：是否存在 / 可执行 / 版本行 / 两个能力标志 `mesh_surface` 和 `mesh_surface_stress`。`run_parse_probe=True` 时在临时目录额外跑一个自包含的 2 三角片 STL + 最小 `mesh/surface/stress` deck (`run 0`)，返回 `mesh_surface_stress_parse_probe`、`parse_probe_exit_code`、`parse_probe_work_dir`、`parse_probe_error_tail`。grep `-help` 的 `mesh_surface_stress` 标志可能不准（某些 build 在 help 里列了命令但 parser 实际拒绝）—— parse probe 是权威判定。plate 流水线必须 `mesh_surface_stress_parse_probe=true`；apt LIGGGHTS 只有 `mesh/surface` 没有 `mesh/surface/stress` |
...
如果某些 build 在 `-help` 里列了 `mesh/surface/stress` 但 parser 实际拒绝，传 `run_parse_probe=True` 做更严格的检查：server 会在临时目录写一个 2 三角片 STL 和一个最小 deck（用了 `fix mesh/surface/stress` 和 `wall/gran ... mesh` 然后 `run 0`），跑一次。整个 probe 是自包含的 —— 不依赖 PINN 目录或项目 STL。临时目录会保留在 `/tmp/liggghts_parse_probe_*` 下方便 debug；`mesh_surface_stress_parse_probe=false` 时先看 `parse_probe_error_tail`。
